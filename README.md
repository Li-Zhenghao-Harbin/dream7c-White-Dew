...
│   │   ├── ProgressDetail.vue  # 进度详情 — 记录表格、统计、面试经验总览
│   │   └── DataManagement.vue  # 数据管理页面
│   │
│   ├── components/
│   │   ├── RecordForm.vue      # 招聘记录表单（新增 / 编辑、流程阶段、面试经验）
...
3. **跟踪流程阶段** — 每条记录可添加多个流程节点（投递 → 笔试 → 一面 → 二面 → HR 面），并记录每个阶段的日期、备注和一段对应的面试经验
4. **更新招聘结果** — 随着进展修改状态：进行中 → offer / 面试挂 / 简历挂等
5. **查看统计分析** — 点击「统计」按钮，通过饼图直观了解各状态比例
6. **整理面试经验** — 点击「面试经验」按钮，可在当前进度下按公司或按时间查看所有已保存的面试经验
7. **数据管理** — 随时备份数据为 JSON 文件，需要时可导入恢复
...
- 主数据保存在 `progresses` 数组中，包含进度、记录和流程阶段。
- 面试经验单独保存在根级 `interviewExperiences` 数组中，并通过 `progressId`、`recordId`、`stageId` 关联到具体阶段。
- 单个阶段最多对应一条面试经验；导入 / 导出时仍然使用同一个 JSON 文件，不需要拆分多个文件。
